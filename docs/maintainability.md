...
`src`, `tools` and `tests`. The generated dataset table modules are excluded
in `maintainability.json`: reviewing a generated declaration as if it were
control-flow code would not help a maintainer.
