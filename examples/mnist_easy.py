...
bytes by - is read off the file by :mod:`xrdclient.ml` and can be printed.
...
``mnist_mlp.py`` is this same program written against :mod:`xrdclient.root.ml`, one
...
with xrdclient.ml.load(URL) as data:
